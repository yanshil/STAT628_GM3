...
summary(base.BIC)
library(boot)
m1<-glm(BODYFAT ~ AGE + ADIPOSITY + NECK + CHEST + ABDOMEN + HIP + FOREARM + WRIST, data = datset)
m2<-glm(BODYFAT ~ AGE + ADIPOSITY + CHEST + ABDOMEN + WRIST,data = datset)
m3<-glm(BODYFAT ~ ABDOMEN + WEIGHT + WRIST + BICEPS + AGE, data = datset)
m4<-glm(BODYFAT ~ ABDOMEN + WEIGHT + WRIST, data = datset)

cv.glm(data=datset,glmfit=m1,K=10)$delta
cv.glm(data=datset,glmfit=m2,K=10)$delta
cv.glm(data=datset,glmfit=m3,K=10)$delta
cv.glm(data=datset,glmfit=m4,K=10)$delta
